...
class StoryPointsStatisticsEvaluatorTest extends FlatSpec with ShouldMatchers {
  ...
  private val item = new Item(null, Duration.standardHours(1), null, created = startDate, resolved = Some(endDate))
  private val otherItem = item.copy(originalEstimate = Duration.standardHours(3), created = endDate)
  private val notEstimatedItem = item.copy(originalEstimate = Duration.standardHours(0), created = endDate)
  ...
  it should "evaluate statistics of items basing on Stories original estimates" in {
    ...
    results.created should be(Set(DataPoint(startDate, 2), DataPoint(endDate, 3)))
